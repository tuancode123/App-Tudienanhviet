...
public class TuVungDAO {
    ...
    public static final String SQL_TU_VUNG = "CREATE TABLE " + TABLE_NAME + " (tuTA TEXT PRIMARY KEY, tuTV TEXT, phienAm TEXT DEFAULT '');";
    ...
    public List<TuVung> searchCharacter(String character) {
        ...
        String sSQL = "SELECT * FROM TuVung WHERE tuTA LIKE '" + character + "%'";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
    ...
    public List<TuVung> getAll() {
        List<TuVung> listTu = new ArrayList<>();
        String sSQL = "SELECT * FROM TuVung ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            TuVung tuVung = new TuVung();
